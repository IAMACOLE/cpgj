...
@interface DoubleChaseViewController ()<UITableViewDelegate,UITableViewDataSource,KeyBoardViewDelegate,DoubleChaseViewDelegate,DoubleChaseTableViewCellDelegate,DoubleChaseFooterViewDelegate>
...
@implementation DoubleChaseViewController
...
-(void)generateChaseClick{
    ...
    for (int i =0 ; i<[self.headView.preiodTextField.text integerValue]; i++) {
        ...
        if(i!=0&&i%[self.headView.modelTextField.text integerValue]==0){
            temp=temp*[self.headView.bonusLabel.text intValue];
        }
        if (temp>1) {
            model.timesStr = [NSString stringWithFormat:@"%ld",[self.headView.timesTextField.text integerValue]*temp];
        }
        else{
             model.timesStr = [NSString stringWithFormat:@"%ld",[self.headView.timesTextField.text integerValue]];
        }
    ...
    [self.headView setAllperiod:self.headView.preiodTextField.text andMoneyStr:[NSString stringWithFormat:@"%.2f",allMoney]];
    [self.tableView reloadData];
}
//取消下单
-(void)cancelClick{
...
//                                 };
    ...
    // 满聪改了几个入参值  bet_times zhuihao_mode zhuihao_bs  zhuihao_qs_bs  zhuihao_jgqs
    NSDictionary * parameter = @{
                                 @"lottery_id" : self.lottery_id,
                                 @"user_token" : token,
                                 @"platform"   : @"2",
                                 ...
                                 @"zhuihao_qh"       :self.headView.preiodTextField.text,
                                 @"zhuihao_stop"     :zhuihao_stop,
                                 @"zhuihao_bs"       :self.headView.bonusLabel.text,
                                 //                  @"zhuihao_qs_bs"    :self.headView.timesTextField.text,
                                 @"zhuihao_ignore_qh":zhuihao_ignore_qh1,
                                 @"zhuihao_jgqs"     :self.headView.modelTextField.text
                                 };
...
-(void)KeyBoardViewSureClick:(NSString *)str{
    ...
    switch (_selectTextFieldStatus) {
            ...
            break;
            case 3:
            self.headView.bonusLabel.text = str;
            break;
            case 4:
            self.headView.modelTextField.text = str;
            break;
        default:
            break;
    }
...
-(DoubleChaseView *)headView{
    if (!_headView) {
        ...
        self.headView.isDoubleChaseView = YES;
